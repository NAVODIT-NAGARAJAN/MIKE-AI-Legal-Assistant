...
export const Register: React.FC = () => {
  ...
  const features = [
    { icon: <ShieldCheck className="w-5 h-5 text-indigo-400" />, title: "Secure & Confidential", desc: "Your data is protected with bank-level security." },
    { icon: <Zap className="w-5 h-5 text-indigo-400" />, title: "AI-Powered Insights", desc: "Instant legal analysis tailored to your specific case." },
    { icon: <BookOpen className="w-5 h-5 text-indigo-400" />, title: "Comprehensive Knowledge", desc: "Built on extensive consumer rights laws and precedents." },
  ];
    ...
    <div className="min-h-screen flex bg-white flex-row-reverse">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-slate-900 relative overflow-hidden flex-col justify-between p-12">
        ...
        <div className="absolute top-0 left-0 w-full h-full opacity-10 pointer-events-none">
           <div className="absolute top-[-10%] right-[-10%] w-[50%] h-[50%] rounded-full bg-indigo-500 blur-[100px]" />
        ...
        <motion.div 
          ...
          className="relative z-10 flex items-center space-x-3 justify-end"
        >
        ...
        <div className="relative z-10 space-y-8 max-w-md ml-auto text-right">
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <h1 className="text-4xl font-bold text-white leading-tight mb-4">
              Join the future of legal guidance.
            </h1>
            <p className="text-slate-300 text-lg">
              Create an account today and take the first step toward resolving your consumer issues.
            </p>
          </motion.div>

          <div className="space-y-6 pt-8 border-t border-slate-800/50 inline-block text-left">
            {features.map((feature, idx) => (
              <motion.div 
                ...
                className="flex items-start space-x-4 flex-row-reverse space-x-reverse text-right"
              >
                <div className="bg-slate-800/50 p-2 rounded-lg border border-slate-700/50 flex-shrink-0 mt-1">
                  {feature.icon}
                </div>
                <div>
